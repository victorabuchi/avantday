...
use lib_ot::core::{
  AttributeHashMap, Body, Changeset, Extension, NodeData, NodeId, NodeOperation, NodeTree,
  NodeTreeContext, Path, Selection, Transaction,
};
...
pub fn make_transaction_from_document_content(content: &str) -> FlowyResult<Transaction> {
  let document_node: DocumentNode =
    serde_json::from_str::<DocumentContentDeserializer>(content)?.document;
  ...
  fn from(node_operation: &NodeOperation) -> Self {
    ...
    match node_operation {
      ...
      NodeOperation::Update { path, changeset } => match changeset {
        Changeset::Delta { delta, inverted } => DocumentOperation::UpdateText {
          path,
          delta,
          inverted,
        },
...
impl<'a> Serialize for DocumentContentSerializer<'a> {
    ...
    S: Serializer,
  {
    ...
    let get_document_node_data =
      |node_id: NodeId| tree.get_node_data(node_id).map(DocumentNode::from);
...
mod tests {
  ...
  fn transaction_deserialize_update_attribute_operation_test() {
    ...
    let json = r#"{"operations":[{"op":"update","path":[0],"attributes":{"retain":3},"oldAttributes":{"retain":3}}]}"#;
    let _ = serde_json::from_str::<DocumentTransaction>(json).unwrap();
  }
