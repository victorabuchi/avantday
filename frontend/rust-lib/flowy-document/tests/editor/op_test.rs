...
fn delta_utf16_code_unit_seek_with_attributes() {
  ...
  delta.add(DeltaOperation::insert_with_attributes(
    "1234",
    attributes.clone(),
  ));
...
fn delta_next_op_len() {
  ...
  assert_eq!(
    iter.next_op_with_len(2).unwrap(),
    DeltaOperation::insert("34")
  );
  assert_eq!(
    iter.next_op_with_len(2).unwrap(),
    DeltaOperation::insert("5")
  );
