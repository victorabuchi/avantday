  ...
  group('toolbar_service.dart', () {
    testWidgets('Test toolbar service in multi text selection', (tester) async {
      ...
      final item = defaultToolbarItems
          .where((item) => item.id == 'avantday.toolbar.link')
          .first;
    ...
    testWidgets(
      ...
      final attributes = BuiltInAttributeKey.partialStyleKeys
        ...
        if (element == BuiltInAttributeKey.backgroundColor) {
          ...
          previousValue[element] = 'avantday.io';
        } else {
      ...
      void testHighlight(bool expectedValue) {
        for (final styleKey in BuiltInAttributeKey.partialStyleKeys) {
          ...
          final itemWidget = _itemWidgetForId(tester, 'avantday.toolbar.$key');
    ...
    testWidgets(
      ...
      expect(find.byType(ToolbarWidget), findsOneWidget);
      var itemWidget = _itemWidgetForId(tester, 'avantday.toolbar.h1');
      ...
      expect(find.byType(ToolbarWidget), findsOneWidget);
      itemWidget = _itemWidgetForId(tester, 'avantday.toolbar.quote');
      expect(itemWidget.isHighlight, true);
      ...
      expect(find.byType(ToolbarWidget), findsOneWidget);
      itemWidget = _itemWidgetForId(tester, 'avantday.toolbar.bulleted_list');
      expect(itemWidget.isHighlight, true);
    });
    ...
    testWidgets('Test toolbar service in multi text selection', (tester) async {
      ...
      expect(find.byType(ToolbarWidget), findsOneWidget);
      expect(
        _itemWidgetForId(tester, 'avantday.toolbar.h1').isHighlight,
        false,
      );
      expect(
        _itemWidgetForId(tester, 'avantday.toolbar.bold').isHighlight,
        false,
      );
